...
<html lang="pt-br">  
	...
	<body>        
		...
		<div id="work_<%=workList.elementAt(i)[0].trim()%>" onclick="SCHED_WORK_START_FUNCTION('<%=workList.elementAt(i)[0].trim()%>','<%=workServiceList.elementAt(0)[0].trim()%>')">
			<%=workList.elementAt(i)[1].trim()%><%if(workList.elementAt(i)[3].trim().compareTo("0") != 0) {%> - INICIADO<%}%>
		</div>
		<canvas id="myCanvas_<%=workList.elementAt(i)[0].trim()%>" width="500" height="1" style="border:0px;"></canvas>
