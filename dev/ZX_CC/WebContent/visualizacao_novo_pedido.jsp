						...
						<%try{
							...
							for(int j=0; j<listVeiculo.size(); j++){
								...
								<%if(AREA.compareTo("CAD_GS")==0){%>
									<br><b>Senha para Atendimento:</b>&nbsp;<%=beanPedido.getSenhaAtendimento(Integer.parseInt(CountUnidadesVeículo.elementAt(i)[0]))%>
								<%}else{%>
									<br><b>Senha para Atendimento:</b>&nbsp;****
								<%}%>
								<fieldset class="fieldinner">
								<legend><b>Contatos para Procedimento:</b></legend>
									<%Vector<String[]> contatoProcedimentoList = beanPedido.getContatoProcedimento(Integer.parseInt(CountUnidadesVeículo.elementAt(i)[0]));
