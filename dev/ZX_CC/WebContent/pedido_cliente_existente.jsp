<!--
ZIRIX CONTROL CENTER - PEDIDO CLIENTE EXISTENTE
...
<%
	...
	if(session.getAttribute("user") == null){
		...
		session.setAttribute("novoCliente", "1");
	}	
%>
<div class="tab-pane active" id="aba_cliente">
	<fieldset class="field">
		...
		<fieldset class="fild_vendedor">
			...
			<%try{
				...
				for (int i=0;i < list.size();i++) {
					...
					if(dao.getPkValueFor("COD_VENDEDOR") == codVendedor){%> selected<%}%>><%=str%></option>
				<%}%>
			<%}catch (Exception e){
		...
		<br>Razão Social / Nome:<input list="nome_list" name="nome_razaosocial_consulta" id="item_nome_razao" class="size_100" onblur="searchCliente()" value="<%=beanCliente.getNome().trim()%>">
...
<div class="tab-pane" id="aba_unidade">
	...
	<legend>Procedimentos em Caso de Emergencia e Alertas:</legend>
		<div id="unidade_procedimento">
			...
			<fieldset class="fieldinner">
			<legend>Inseridos:</legend>
				<div id="contato_procedimento_inserido"></div>
			</fieldset>
			...
			<%try{
				...
				for (int i=0;i < list.size();i++){
					...
					String str = String.valueOf(dao.getAttValueFor("NOME")).trim();
					if(str.compareTo("Estoque") != 0){%> <option value="<%=dao.getPkValueFor("COD_UNIDADE")%>"><%=str%></option><%}
				}
			}catch (Exception e){
				out.println("Error... " + e.getMessage());
			}%>
			</select>
			<div id="tipo_unidade"></div>
		</div>
		<fieldset class="fieldinner">
		<legend>Inseridos:</legend>
			<div id="unidades_inseridas"></div>
		</fieldset>
...
<div class="tab-pane" id="aba_servicos">
	...
	<legend>Equipamentos / Acessórios:</legend>
		...
		<fieldset class="fieldinner">
		<legend>Inseridos:</legend>
			<div id="equip_acessorio_inserido"></div>
		</fieldset>
	...
	<legend>Serviços:</legend>
		...
		<fieldset class="fieldinner">
		<legend>Inseridos:</legend>
			<div id="serv_monit_inserido"></div>
		</fieldset>
