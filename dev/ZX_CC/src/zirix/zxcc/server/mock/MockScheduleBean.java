...
public class MockScheduleBean {
	...
	public MockScheduleBean() {

	}

	public void setPk(Object pkVal) {
		COD_USUARIO_ = new Integer((String)pkVal);
	}
	...
	public Vector<String[]> getWork(){
		...
		try {
			ArrayList<Object[]> values = DAOManager.getInstance().executeQuery("SELECT " + ZXMain.DB_NAME_ + "SCHED_WORK.WORK_ID, " + ZXMain.DB_NAME_ + "SCHED_WORK.WORK_NAME "
					+ "     , " + ZXMain.DB_NAME_ + "SCHED_PROCESS.PROCESS_NAME, ISNULL(" + ZXMain.DB_NAME_ + "SCHED_WORK.COD_USUARIO,0) "
					...
					+ "   AND (" + ZXMain.DB_NAME_ + "SCHED_WORK.START_TIMESTAMP IS NULL OR " + ZXMain.DB_NAME_ + "SCHED_WORK.COD_USUARIO = " + COD_USUARIO_ + ") "
	...
	public void setStartTimestamp(String work_id){
		   if(work_id.compareTo("0") != 0){
			   try {
				   DAOManager.getInstance().executeQuery("UPDATE " + ZXMain.DB_NAME_ + "SCHED_WORK SET START_TIMESTAMP = GETDATE(), COD_USUARIO = " + COD_USUARIO_
